...
The short URLs generated by the system will have seven characters of base62 meaning we would have 62^7 possible combinations ensuring we will most likely not run out of short URLs. However, one must note that when our database reaches 50% capacity, performance will likely downgrade because we will have frequent collisions. For example, with 1 million generated URLs, we will have ~13% chance of collisions. Read more [here](https://en.wikipedia.org/wiki/Birthday_problem). 
